...
class PdfxXmlReader < BioInterchange::TextMining::TMReader
  ...
  def deserialize(inputstream)
    ...
    raise BioInterchange::Exceptions::ReaderError, 'InputStream not of type IO, cannot read.' unless inputstream.kind_of?(IO) or inputstream.kind_of?(String)
...
private
  ...
  def pdfx
    ...
    def tag_start(name, attr)
      #puts "tag_start: #{name}"
      if name =~ /^job$/
        raise BioInterchange::Exceptions::ReaderError, 'Document XML has multiple <job> tags, cannot parse multiple documents within a single file.' if @map['id_done']
        @map['id'] = true
      elsif name =~ /^article-title$/
        raise BioInterchange::Exceptions::ReaderError, 'Document XML has multiple <article-title> tags defined, cannot parse multiple documents within a single file.' if @map['title_done']
        @map['title'] = true
        @map['title_s'] = @map['art_l']
        @map['title_l'] = 0
      elsif name =~ /^abstract$/
        raise BioInterchange::Exceptions::ReaderError, 'Document XML has multiple <abstract> tags defined, cannot parse multiple documents within a single file.' if @map['abs_done']
        @map['abs'] = true
        @map['abs_s'] = @map['art_l']
        @map['abs_l'] = 0
      elsif name =~ /^body$/
        raise BioInterchange::Exceptions::ReaderError, 'Document XML has multiple <body> tags defined, cannot parse multiple documents within a single file.' if @map['body_done']
        @map['body'] = true
        @map['body_s'] = @map['art_l']
        @map['body_l'] = 0
      elsif name =~ /^article$/
        raise BioInterchange::Exceptions::ReaderError, 'Document XML has multiple <article> tags defined, cannot parse multiple documents within a single file.' if @map['art_done']
        @map['art'] = true
        @map['art_s'] = 0
        @map['art_l'] = 0
      elsif name =~ /^section$/
        raise BioInterchange::Exceptions::ReaderError, 'Error with section stack, stacks not equal in size' unless  @map['sec_s'].size == @map['sec_l'].size
