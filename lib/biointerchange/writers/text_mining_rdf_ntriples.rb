...
class RDFWriter < BioInterchange::Writer
  ...
  def serialize(model)
    if model.instance_of?(BioInterchange::TextMining::Document) then
      ...
      raise BioInterchange::Exceptions::WriterError, 'The provided model cannot be serialized at the moment. ' +
...
private
  ...
  def process_uri(process, kind)
    ...
    when :date
      ...
      raise BioInterchange::Exceptions::WriterError, "There is no implementation for serializing a process as #{kind}."
    end
  end
  ...
  def content_uri(content, kind)
    ...
    #  RDF::URI.new("#{base_uri}/type/#{content.uri.sub(/^.*?:\/\//, '')}")
      ...
      raise BioInterchange::Exceptions::WriterError, "There is no implementation for serializing a content as #{kind}."
    end
  end
